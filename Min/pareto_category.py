...
#######################################################################################################################
# Filter for Finance-related jobs
finance_keywords = ["Finance", "Banking", "Investment", "Accounting", "Fintech"]
df_finance = df_exploded[df_exploded["Skills_Industry"].str.contains('|'.join(finance_keywords), case=False, na=False)]
# Count occurrences of each Salary Bin within Finance-related jobs
salary_finance_counts = df_finance.groupby(["Salary Bin", "Skills_Industry"]).size().unstack(fill_value=0)

# Compute total job counts per salary bin
salary_total_counts = salary_finance_counts.sum(axis=1)
...
# Plot stacked bar chart for salary bins in Finance industry
fig, ax1 = plt.subplots(figsize=(10, 6))

# Stacked bar chart
salary_finance_counts.plot(kind="bar", stacked=True, ax=ax1, colormap="coolwarm", alpha=0.8)
ax1.set_ylabel("Number of Jobs")
ax1.set_title("Pareto Chart of Salary Range in Finance Industry")
...
# Show legend
ax1.legend(title="Finance Category", bbox_to_anchor=(1.05, 1), loc="upper left")
...
# Plot stacked bar chart for salary bins by industry/skill
fig, ax1 = plt.subplots(figsize=(10, 6))

# Stacked bar chart
salary_skill_counts.plot(kind="bar", stacked=True, ax=ax1, colormap="tab10", alpha=0.8)
ax1.set_ylabel("Number of Jobs")
