...
it_keywords = ["Artificial Intelligence", "Cloud", "Software", "Blockchain", "Cybersecurity", 
               "Machine Learning", "DevOps", "Data Science", "Big Data", "IT", "Networking"]
df_it = df_exploded[df_exploded["Skills_Industry"].str.contains('|'.join(it_keywords), case=False, na=False)]

# Count occurrences of each Salary Bin within IT-related jobs
salary_it_counts = df_it.groupby(["Salary Bin", "Skills_Industry"]).size().unstack(fill_value=0)

# Compute total job counts per salary bin
salary_total_counts = salary_it_counts.sum(axis=1)
...
# Plot stacked bar chart for salary bins in IT industry
fig, ax1 = plt.subplots(figsize=(10, 6))

# Stacked bar chart
salary_it_counts.plot(kind="bar", stacked=True, ax=ax1, colormap="viridis", alpha=0.8)
ax1.set_ylabel("Number of Jobs")
ax1.set_title("Pareto Chart of Salary Range in IT Industry")
...
# Show legend
ax1.legend(title="IT Category", bbox_to_anchor=(1.05, 1), loc="upper left")
